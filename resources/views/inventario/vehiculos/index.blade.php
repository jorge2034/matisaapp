...
@section('breadcrumbs')
    {{ Breadcrumbs::render('invVehiculos') }}
@endsection
@section('content')
    <div class="container-fluid">
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                {{--Filtros--}}
               @include('inventario.vehiculos.partials.filter')

                <div class="card card-primary card-outline">
                    <div class="card-header h5">
                        Resultados
                        @can('invvehiculos.create')
                        <a href="{{route('inventario.vehiculos.create')}}" class="btn btn-primary btn-sm pull-right">Crear</a>
                        @endcan
                    </div>
                          ...
                          @foreach($invVehiculos as $invVehiculo)
                              <tr>
                                  ...
                                  <td width="100px" class="text-center">
                                      <div class="accion">
                                              @can('invvehiculo.show')
                                              <a href="{{route('inventario.vehiculos.show',$invVehiculo)}}"
                                                 title="Ver" class="btn btn-sm  btn-accion">
                                                  <i class="fa fa-eye"></i>
                                              </a>
                                              @endcan
                                              @can('invvehiculo.edit')
                                              <a href="{{route('inventario.vehiculos.edit',$invVehiculo->id)}}"
                                                 title="Modificar" class="btn btn-sm  btn-accion">
                                                  <i class="fa fa-edit"></i>
                                              </a>
                                              @endcan
                                              @can('invvehiculo.destroy')
                                              <a href="#" onclick="borrar({{$invVehiculo->id}})" title="Eliminar" class="btn btn-sm btn-borrar btn-accion">
